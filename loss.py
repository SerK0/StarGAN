...
class GeneratorLoss:
    ...
    def __call__(self, real, fake, reconstructed, labels) -> torch.Tensor:
        """
        ...
        :param torch.Tensor labels: true labels from dataset
        :rtype: torch.Tensor
        :returns: Overal Generator Loss ready for backpropagate
        """
        output_src, output_cls = self.discriminator(fake)

        loss_adv = self.adversarial_loss(fake_logits=output_src, real_logits=None)
        loss_dm_cls = self.domain_classification_loss(
            logit=torch.squeeze(output_cls), target=labels
        )
...
class DiscriminatorLoss:
    ...
    def __call__(self, real, fake, reconstructed):
        pass